...
def Clean_data(raw_data):
    data_cleaned = raw_data.drop_duplicates()
    return data_cleaned


def process_data(
        data, categorical_features, label, training=True,
...
def save_model(model, encoder, lb,
               model_filepath, encoder_filepath, lb_filepath):
...
def compute_slice_metrics(model, X_test, y_test, feature_name, feature_index):
    ...
    for value in unique_values:
        ...
        precision = precision_score(slice_labels, predictions,
                                    average='weighted', zero_division=0)
        recall = recall_score(slice_labels, predictions,
                              average='weighted', zero_division=0)
        f1 = f1_score(slice_labels, predictions, average='weighted',
                      zero_division=0)
...
if __name__ == "__main__":
    ...
    metrics_df = compute_slice_metrics(model, X_test, y_test,
    ...
    save_model(model, encoder, lb, "../model/model.joblib",
               "../model/encoder.joblib", "../model/label_binarizer.joblib")
