...
def run_sanity_check(test_dir):
    ...
    file_prompt = """ This script will perform a sanity test to ensure your
    code meets the criteria in the rubric. Please enter the path to the file
    that contains your test cases for the GET() and POST() methods. The path
    should be something like abc/def/test_xyz.py """
    print(file_prompt)
    filepath = input('> ')
    ...
    if not test_functions_for_get:
        ...
        print(FAIL_COLOR+"\nPlease make sure you have a test case for the GET\
            method. This MUST test both the status code as well as the contents\
    ...
    if not test_functions_for_post:
        ...
        print(FAIL_COLOR+"Please make sure you have TWO test cases for the POST()\
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_CODE:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"One or more of your test cases for POST() do not seem to be\
                   testing the response code.\n")
        ...
        if len(test_functions_for_post) >= 2 and COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR+"You do not seem to have TWO separate test cases, one for each\
    ...
    print(WARN_COLOR+"You should still check your work against the rubric to ensure you\
           meet the criteria.")
